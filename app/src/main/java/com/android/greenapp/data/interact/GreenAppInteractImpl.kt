...
class GreenAppInteractImpl @Inject constructor(
	...
	override suspend fun getAvailableLanguageList(): Resource<List<LanguageItem>> {
		try {
			...
			VLog.d("Exception in downloading available language list : $timeOutEx")
			return Resource.error(timeOutEx)
		} catch (ex: java.lang.Exception) {
			VLog.d("Exception in downloading lang list : ${ex.message}")
			return Resource.error(Exception(ex))
		}
		return Resource.error(Exception("Exception in downloading language list"))
	}
	...
	override suspend fun downloadLanguageTranslate(langCode: String): Resource<String> {
		try {
			...
			VLog.d("TimeOutException in downloading language content : $timeOut")
			return Resource.error(timeOut)
		} catch (ex: java.lang.Exception) {
			VLog.d("Failed getting language string resource and saving it in db : ${ex.message}")
		}
		return Resource.error(Throwable("Unknown exception in downloading chosen lang"))
	}
	...
	override suspend fun getAllNetworkItemsListFromPrefs(): Resource<List<NetworkItem>> {
		...
		return Resource.error(Throwable("AllNetworkItemList is empty"))
	}
	...
	override suspend fun requestOtherNotifItems() {
		try {
			...
			if (res.isSuccessful) {
				...
				val listNotifOtherItems = mutableListOf<NotifOtherEntity>()
				for (at in 0 until otherNotifItemsJsonArray.length()) {
					...
					listNotifOtherItems.add(notifOther)
				}
				val allNotifItemsList =
					notifOtherDao.getALlNotifOthersByGivenParameters(null, null, null)
				VLog.d("All NotifItems List in DB : $allNotifItemsList")
				for (notifEntity in listNotifOtherItems) {
					val existInDb = notifOtherDao.getNotifOtherItemByGuid(notifEntity.guid)
					if (!existInDb.isPresent && allNotifItemsList.isNotEmpty()) {
						notifHelper.callGreenAppNotificationMessages(
							notifEntity.message,
							notifEntity.created_at_time
						)
					}
				}
				val affectedRows = notifOtherDao.insertOtherNotifItems(listNotifOtherItems)
				VLog.d("Affected Rows counter from  inserting it to db : $affectedRows")
			} else {
	...
	override suspend fun getAgreementsText(): Resource<String> {
		...
		return Resource.error(Exception("Error in gettingAgreement text"))
	}
		...
		try {
			...
			if (res.isSuccessful) {
				...
				for (coinJsonElement in coinList) {
					...
					val fee_commission = jsonObject.getDouble("fee_transaction")
