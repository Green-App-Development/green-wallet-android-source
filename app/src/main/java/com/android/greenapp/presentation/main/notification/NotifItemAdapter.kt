        ...
        fun onBind(notificationItem: NotificationItem) {
            ...
            if (notificationItem.message.isEmpty()) {
                ...
                if (notificationItem.status == Status.Outgoing) {
                    txtDescription.text =
                        trimNetwork(activity.getStringResource(R.string.notifications_withdrawn_from_wallet)
                    ...
                    txtAmount.apply {
                        ...
                        text =
                            "-${formattedDoubleAmountWithPrecision(notificationItem.amount)} ${
                                getShortNetworkType(
                                    notificationItem.networkType
                                )
                            }"
                    }
                } else if (notificationItem.status == Status.Incoming) {
                    txtDescription.text =
                        trimNetwork(activity.getStringResource(R.string.notifications_credited_to_wallet)
                            .replace("%network%", notificationItem.networkType))
                    txtAmount.text =
                        "+${formattedDoubleAmountWithPrecision(notificationItem.amount)} ${
                            getShortNetworkType(
                ...
                txtAmountInUSD.text =
                    "${formattedDollarWithTwoDecimal(notificationItem.amountInUSD)}$"
