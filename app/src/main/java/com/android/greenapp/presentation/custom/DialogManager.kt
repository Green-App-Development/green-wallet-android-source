...
//            findViewById<TextView>(R.id.txtDescription).text = description
	...
	fun showTimeOutException(activity: Activity,callback: () -> Unit) {
		activity.apply {
			showFailureDialog(
				this,
				getStringResource(R.string.no_connection_title),
				getStringResource(R.string.no_connection_description),
				getStringResource(R.string.return_btn)
			) {
				callback()
			}
