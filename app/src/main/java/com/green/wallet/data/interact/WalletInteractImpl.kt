...
import com.green.wallet.presentation.custom.encryptor.EncryptorProviderImpl.Companion.FIRST_STAGE
...
class WalletInteractImpl @Inject constructor(
    ...
    private suspend fun getDecryptedMnemonicsList(
        encMnemonics: String,
        address: String
    ): List<String> {
        try {
            val key =
                prefsInteract.getSettingString(PrefsManager.PASSCODE, "")
            var decMnemonics = encryptor.decrypt(encMnemonics, key)
            if (decMnemonics.isEmpty()) {
                encryptor.setStage(FIRST_STAGE)
                decMnemonics = encryptor.decrypt(encMnemonics, key)
                encryptor.setStage(SECOND_STAGE)
                val encrypted = encryptor.encrypt(decMnemonics, key)
                walletDao.updateMnemonicsAndEncStage(encrypted, SECOND_STAGE, address)
            }
...
//				VLog.d("TokenPrice for $token is $tokenPrice in walletInteractImpl")
    ...
    override suspend fun getWalletByFingerPrint(id: Long) =
        walletDao.getWalletByFingerPrint(id)
            .map { it.toWallet(getDecryptedMnemonicsList(it.encMnemonics, it.address)) }
    ...
    private suspend fun convertWalletEntityToWalletWithAmountInUSD(it: WalletEntity): Wallet {
        ...
        val wallet = it.toWallet(getDecryptedMnemonicsList(it.encMnemonics, it.address))
    ...
    override suspend fun checkIfMnemonicsExistInDB(
        ...
        val encMnemonics = encryptor.encrypt(mnemonics.toString(), key)
    ...
    override suspend fun getWalletByAddress(address: String): Wallet {
        ...
        val wallet =
            walletEntity.toWallet(getDecryptedMnemonicsList(walletEntity.encMnemonics, address))
        return wallet
    }
