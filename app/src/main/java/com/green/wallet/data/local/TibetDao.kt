...
interface TibetDao {
	...
	@Query("SELECT * FROM TibetLiquidityEntity WHERE offer_id=:offer_id")
	fun getTibetLiquidityEntityByOfferId(offer_id: String): Flow<TibetLiquidityEntity>
	...
	@Query("UPDATE TibetLiquidityEntity SET height=:height, status=:status WHERE offer_id=:offer_id")
	suspend fun updateTibetLiquidityHeightStatus(
		height: Int,
		status: OrderStatus,
		offer_id: String
	): Int
