...
interface TibetDao {
	...
	suspend fun getTibetLiquidityStatusInProgress(status: OrderStatus = OrderStatus.InProgress)

	@Query("UPDATE TibetLiquidityEntity SET height=:height AND status=:status WHERE ")
