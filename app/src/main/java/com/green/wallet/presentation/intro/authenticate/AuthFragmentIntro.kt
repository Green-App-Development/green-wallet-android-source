...
//		}
    ...
    private fun verifiedPasscode(passCode: IntArray) {
        lifecycleScope.launch {
            ...
            val decrypted =
                introActViewModel.encryptor.decrypt(savedPassCode, BuildConfig.KEY_PASSCODE)
            VLog.d("Decrypted passcode value : $decrypted")
            if (passCode.toList().toString() == savedPassCode || passCode.toList()
                    .toString() == decrypted
            ) {
                determineDestination()
            } else {
