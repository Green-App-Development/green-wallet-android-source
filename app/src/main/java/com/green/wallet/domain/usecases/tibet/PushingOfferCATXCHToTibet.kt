...
class PushingOfferCATXCHToTibet @Inject constructor(
	...
	suspend operator fun invoke(
		...
		val res = tibetInteract.pushOfferToTibetCAT(
			pair, offer, ACTION_SWAP, donation_amount,
			devFee, walletFee
		)
		if (res.state == Resource.State.SUCCESS) {
			...
			spentCoinsInteract.insertSpentCoinsJson(spentCATCoins, timeCreated, catCode, fk_address)
