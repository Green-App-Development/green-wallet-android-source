...
import extralogic.wallet.greenapp.presentation.custom.dialogs.NetworkAdapter
...
fun getStatusBarBcgColorBasedOnTime(): Boolean {
	...
	val isLightBar = when {
		curMinutes in 0..extralogic.wallet.greenapp.presentation.tools.MORNING_STARTS_MINUTE -> false
		curMinutes in extralogic.wallet.greenapp.presentation.tools.MORNING_STARTS_MINUTE..extralogic.wallet.greenapp.presentation.tools.NOON_STARTS_MINUTE -> false
		curMinutes in extralogic.wallet.greenapp.presentation.tools.NOON_STARTS_MINUTE..extralogic.wallet.greenapp.presentation.tools.EVENING_STARTS_MINUTE -> true
		else -> false
	}
	return isLightBar
}
...
fun getStatusBcgBarColorBasedOnTime(activity: Activity): Int {
	val curMinutes = getCurrentTimeInMinutes()
	val color = when {
		curMinutes in 0..extralogic.wallet.greenapp.presentation.tools.MORNING_STARTS_MINUTE -> activity.getColorResource(
			R.color.night_status_bar_clr
		)
		curMinutes in extralogic.wallet.greenapp.presentation.tools.MORNING_STARTS_MINUTE..extralogic.wallet.greenapp.presentation.tools.NOON_STARTS_MINUTE -> activity.getColorResource(
			R.color.morning_status_bar_clr
		)
		curMinutes in extralogic.wallet.greenapp.presentation.tools.NOON_STARTS_MINUTE..extralogic.wallet.greenapp.presentation.tools.EVENING_STARTS_MINUTE -> activity.getColorResource(
