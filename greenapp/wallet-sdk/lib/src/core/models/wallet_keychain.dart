...
class WalletKeychain with ToBytesMixin {
  ...
  void addOuterPuzzleHashesForAssetId(Puzzlehash assetId) {
    ...
    final entriesToAddSecond = <Puzzlehash, WalletVector>{};
    for(final  walletVector in hardenedMap.values){
         final outerPuzzleHash=makeOuterPuzzleHash(walletVector.puzzlehash, assetId);
         entriesToAddSecond[outerPuzzleHash]=walletVector;
    }
    hardenedMap.addAll(entriesToAddSecond);
  ...
  static Puzzlehash makeOuterPuzzleHash(
    ...
    final solution = Program.list([
      Program.fromBytes(myCatProgram.hash()),
...
class WalletPuzzlehash extends Puzzlehash {
  WalletPuzzlehash(List<int> bytesList, this.derivationIndex)
      : super(bytesList);
